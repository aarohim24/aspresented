...
<img src="docs/hero.svg" alt="Six charges of Rs 5.00, each inside the mandate's per-charge ceiling. The rail alone settles Rs 30.00 against an authorised total of Rs 20.00. The gate stops at Rs 20.00, refusing two with CUMULATIVE_EXCEEDED." width="100%">

<sup>Not drawn — generated by `python3 tools/gen_hero.py`, which charges a rail
and a gate and plots what each let through. CI regenerates it, so it cannot
flatter the code.</sup>
...
**Who that log convinces is stated precisely rather than generously.** Signing
here is symmetric, so the record proves nobody altered it after the fact — it
does not prove to an adversarial third party *who wrote it*, because the party
presenting the evidence also holds the key. That makes two things sound today:
enforcement, where the merchant is constraining agents it is already
responsible for, and **delegation between agents**, where every party is inside
one trust boundary. Merchant-versus-cardholder adjudication needs asymmetric
issuance, which is named under [Still to come](#still-to-come) rather than
implied here. A dispute console that overstated what it could prove would be
the same mistake this project exists to point out.
...
The block below is the whole integration surface — there is no other setup. An
adapter normalises whatever the rail gave you; `with_policy` attaches what the
rail has no field for; the gate enforces the union and logs the decision.

```python
import time
from mandate_gate import ChargeRequest, Gate, Intent, Ledger, Limits, RailSimulator, Scope
from mandate_gate.adapters.razorpay_upi import RazorpayUpiAdapter

now = int(time.time())

# Everything a Razorpay UPI mandate can hold: a ceiling, an expiry, a cadence
# label, a type. No total, no count, no payee.
mandate = {"id": "order_x", "customer_id": "cust_x", "token": {
    "max_amount": 500, "expire_at": now + 30 * 86400,
    "frequency": "as_presented", "type": "single_block_multiple_debit"}}

envelope = RazorpayUpiAdapter.normalise(mandate).with_policy(Limits(
    cumulative_max=2000,                     # no rail has a field for this
    max_charges=8,
    scope=Scope(merchants=frozenset({"shop-a"})),
    requires_intent_binding=True))

print("policy supplies:", sorted(str(c) for c in envelope.unenforced_by_rail))

gate = Gate(envelope, Ledger("decisions.jsonl"),
            RailSimulator(limits=envelope.rail),      # rail limits, never effective
            intent_secret=b"keep-this-in-a-kms", clock=time.time)

intent = gate.record_intent(Intent(
    intent_id="int_1", mandate_id=envelope.mandate_id,
    max_amount=500, expires_at=now + 86400, merchant="shop-a"))

for i, amount in enumerate([500, 490, 480, 470, 460]):   # 2400 paise of shopping
    d = gate.authorize(ChargeRequest(
        mandate_id=envelope.mandate_id, amount=amount, idempotency_key=f"key-{i}",
        intent_id=intent.intent_id, merchant="shop-a"))
    if d.allowed:
        print(f"{amount} paise: allowed")
    else:
        print(f"{amount} paise: REFUSED {d.refusals[0].code} -- {d.refusals[0].detail}")
        print(f"          {d.refusals[0].remediation}")
```

```
policy supplies: ['cumulative_max', 'intent_binding', 'max_charges', 'scope']
500 paise: allowed
490 paise: allowed
480 paise: allowed
470 paise: allowed
460 paise: REFUSED CUMULATIVE_EXCEEDED -- 2400 would exceed total cap 2000 (1940 already used)
          Charge at most 60 paise.
```

Three things in that output are the whole argument. `cumulative_max` is a
constraint **no payment network enforces**, so without this layer the fifth
charge settles. The refusal names the field and the fix, so an agent that reads
it can complete the purchase instead of retrying blindly. And `decisions.jsonl`
is now a hash-chained record of all five decisions, replayable by the
adjudicator with nothing else trusted.

Amounts are integer paise, always. Charges are spaced by amount here because
identical repeats trip `DUPLICATE_CHARGE` — retry hygiene, not a spending
control, and [documented as such](#an-attacker-that-never-read-the-checks).

**This snippet is executed by the test suite.** `tests/test_readme.py` extracts
this block, runs it, and compares against the output above, so a README that
lies about its own API fails CI.
...
tools/gen_hero.py        regenerates the README figure, by measuring
...
evidence/                live API findings and recorded model runs
docs/hero.svg            generated, not drawn
tests/                   238 tests, stdlib only
```
...
python3 tools/gen_hero.py                                              # the figure
...
- **Asymmetric issuance**, and it is first because one use case rests on it. A
  principal signing with a private key, so a merchant can verify an authority
  without being able to mint one. Everything here works today where issuer and
  verifier share a trust boundary — a merchant constraining its own agents, an
  agent subcontracting to another agent. Adjudication *against* the principal is
  the case symmetric signing cannot close, and it is scoped out of the claims
  above rather than glossed. Not hand-rolled: this needs an audited Ed25519
  implementation, and inventing signature code inside a project about provable
  restraint would be the joke writing itself.
